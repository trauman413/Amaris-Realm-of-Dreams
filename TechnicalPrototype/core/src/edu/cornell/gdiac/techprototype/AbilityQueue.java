...
public class AbilityQueue {
    ...
    protected int QUEUE_START_X = 10;
    protected int QUEUE_START_Y = 550;
    ...
    public void draw(GameCanvas canvas) {
        ...
        for (FountainModel fountain : abilities) {
            if (fountain.getFountainType() == FountainModel.FountainType.DASH) {
                canvas.draw(dashAbilityTexture, color, x, y, CARD_WIDTH, CARD_HEIGHT);
                y -= CARD_HEIGHT;
            } else if (fountain.getFountainType() == FountainModel.FountainType.TRANSPARENCY) {
                canvas.draw(transparentAbilityTexture, color, x, y, CARD_WIDTH, CARD_HEIGHT);
                y -= CARD_HEIGHT;
            } else if (fountain.getFountainType() == FountainModel.FountainType.FLIGHT){
                canvas.draw(flightAbilityTexture, color, x, y, CARD_WIDTH, CARD_HEIGHT);
                y -= CARD_HEIGHT;
            }
        }
    }
