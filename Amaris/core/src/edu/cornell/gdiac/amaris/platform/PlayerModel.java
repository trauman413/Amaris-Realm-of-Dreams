...
public class PlayerModel extends CapsuleObstacle {
     ...
     */
    private static final float DUDE_DENSITY = 1.9f;
    /**
     ...
     * The maximum character speed
     */
    private static final float DUDE_DASH_MAXSPEED = 17f;
    /**
     ...
     */
    private static final float DUDE_JUMP = 23.5f;
    /**
     * The impulse for the character to fly
     */
    private static final float DUDE_FLIGHT = 55.0f;
    /**
     ...
     * Cooldown (in animation frames) for jumping
     */
    private static final float MAX_FLIGHT_WALK_SPEED = 2.5f;
    /**
     * Ability timer
     */
    ...
    /**
     * Whether the player has been detected to be on a moving platform this frame
     */
    ...
    public void setJumpState(boolean val) {
        elapsedTime = 0;
        if (val) {
            ps = playerState.JUMP;
        }
    ...
    public void setDashState(boolean val) {
        if (val) {
            ps = playerState.DASH;
        }
        else
    ...
    public void setFlightState(boolean val) {
        if (val) {
            ps = playerState.FLIGHT;
        }
        else
            ps = playerState.IDLE;
    }
    ...
    public void setOnMovingPlatform(boolean value) { onMovingPlatform = value; }
    ...
    public void setPressedAbility(boolean pt) {
        pressedAbility = pt;
    }
...
//        System.out.println("height" + height*DUDE_VSHRINK); //1.875
    ...
    public void applyForce() {
        ...
        if (getMovement() == 0f && limitPlayerMotion) {
        ...
        if (Math.abs(getVX()) >= getMaxSpeed() && limitPlayerMotion) {
        ...
        } else if (isDashing() || abilityController.isApplyingDash() || !limitPlayerMotion) {
            ...
            if (getVX() != Math.signum(getMovement()) * DUDE_DASH_MAXSPEED) {
                forceCache.set(getMovement(), 0);
                body.applyForce(forceCache, getPosition(), true);
            }
        }

        if (abilityController.isAbilityActive(FountainModel.FountainType.FLIGHT) && !isGrounded() && isFlying()){
            last = true;
            if (Math.abs(getVX()) >= MAX_FLIGHT_WALK_SPEED){
                setVX(Math.signum(getMovement()) * MAX_FLIGHT_WALK_SPEED);
            }
        } else {
            if (last){
                if(!isGrounded()){
                    if(!abilityController.isAbilityActive(FountainModel.FountainType.FLIGHT) && abilityController.isUsingAbility()){
                        last = false;
                    }
                    if (Math.abs(getVX()) >= MAX_FLIGHT_WALK_SPEED){
                        setVX(Math.signum(getMovement()) * MAX_FLIGHT_WALK_SPEED);
        ...
        if (isDashing() && abilityController.getTimeLeftForAbility() > 0 && !abilityController.isApplyingDash()) {
            abilityController.doDash(this, 300);
        }
        if (abilityController.isAbilityActive(FountainModel.FountainType.DASH) && isDashing()) {
...
//            color = Color.RED;
        ...
        if (effect == 1) {
            canvas.draw(currentFrame, color, origin.x, origin.y, getX() * drawScale.x + 125, getY() * drawScale.y + 80,
           // canvas.draw(currentFrame, color, origin.x, origin.y, getX() * drawScale.x + 100, getY() * drawScale.y + 60,
            ...
            canvas.draw(currentFrame, color, origin.x, origin.y, getX() * drawScale.x + 195, getY() * drawScale.y + 80,
                    effect * texture.getRegionWidth() / FRAME_COLS, texture.getRegionHeight() / FRAME_ROWS);
        }
    }
