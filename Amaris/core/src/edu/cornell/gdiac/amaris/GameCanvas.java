...
//        spriteCam.setToOrtho(false);
		...
		initialX = spriteCam.position.x;
		initialY = spriteCam.position.y;
		initialZoom = spriteCam.zoom;
	}
	...
	public void clear() {
		// Clear the screen
		Gdx.gl.glClearColor(28/255.0f, 21/255.0f, 44/255.0f, 1.0f);
	 ...
	 */
	public void begin(float x, float y, float width) {
		// We are drawing
		active = DrawPass.STANDARD;
		...
		// spriteCam.setToOrtho(false, width, height);
		spriteCam.update();
		spriteCam.position.x = Math.max(Math.min(x, width - getWidth()*initialZoom/(2.0f)), getWidth()*initialZoom/2.0f);
		//spriteCam.position.y = Math.max(Math.min(spriteCam.position.y, 1080 - getHeight()/2.0f - 145), getHeight()/2.0f + 145);
		spriteCam.position.y = Math.max(Math.min(y, 1080 - getHeight()*initialZoom/2.0f + 130), getHeight()*initialZoom/2.0f);
//		initialX = spriteCam.position.x;
//		initialY = spriteCam.position.y;
//		initialZoom = spriteCam.zoom;
//		if (y >= getHeight()/2.0f) {
...
//		}
	 ...
	 * @param y 	 The y-coordinate of the bottom left corner
	 */
	public void drawConstantBackground(Texture image, float x, float y) {
		if (active != DrawPass.STANDARD) {
			Gdx.app.error("GameCanvas", "Cannot draw without active begin()", new IllegalStateException());
			return;
		}
		constantBatch.draw(image, x,   y);
	}
	 ...
	 * @param constant if the font should be drawn moving on the screen.
	 */
	public void drawText(String text, BitmapFont font, float x, float y, boolean constant) {
		if (active != DrawPass.STANDARD) {
			Gdx.app.error("GameCanvas", "Cannot draw without active begin()", new IllegalStateException());
			return;
		}
	 ...
	 * @param y 	The y-coordinate of the bottom left corner
	 */
	public void drawConstant(Texture image, Color tint, float x, float y) {
		if (active != DrawPass.STANDARD) {
			Gdx.app.error("GameCanvas", "Cannot draw without active begin()", new IllegalStateException());
			return;
		}
	 ...
	 */
	public void drawConstant(Texture image, Color tint, float ox, float oy,
		...
		drawConstant(holder,tint,ox,oy,x,y,angle,sx,sy);
	}

	public void drawConstant(TextureRegion region, float x, float y) {
		if (active != DrawPass.STANDARD) {
			Gdx.app.error("GameCanvas", "Cannot draw without active begin()", new IllegalStateException());
			return;
		}

		constantBatch.setColor(Color.WHITE);
		constantBatch.draw(region, x,  y);
	}
	 ...
	 * @param sy 	The y-axis scaling factor
	 */
	public void drawConstant(TextureRegion region, Color tint, float ox, float oy,
							 float x, float y, float angle, float sx, float sy) {
		if (active != DrawPass.STANDARD) {
			Gdx.app.error("GameCanvas", "Cannot draw without active begin()", new IllegalStateException());
			return;
		...
		constantBatch.setColor(tint);
		constantBatch.draw(region, region.getRegionWidth(), region.getRegionHeight(), local);
	}
	 ...
	 * @param height The texture height
	 */
	public void drawConstant(Texture image, Color tint, float x, float y, float width, float height) {
		if (active != DrawPass.STANDARD) {
			Gdx.app.error("GameCanvas", "Cannot draw without active begin()", new IllegalStateException());
			return;
		}
		...
		constantBatch.setColor(tint);
		constantBatch.draw(image, x,  y, width, height);
	}
	/**
	 * Draws the tinted texture at the given position.
	 *
	 ...
	 * @param height The texture height
	 */
	public void drawConstant(TextureRegion region, Color tint, float x, float y, float width, float height) {
		if (active != DrawPass.STANDARD) {
			Gdx.app.error("GameCanvas", "Cannot draw without active begin()", new IllegalStateException());
			return;
		}
	...
	public void beginDebug(Affine2 affine) {
		...
		global.mulLeft(spriteCam.combined);
	...
	public void beginDebug() {
		debugRender.setProjectionMatrix(spriteCam.combined);
	...
	public void savePosition() {
		prevX = spriteCam.position.x;
		prevY = spriteCam.position.y;
	}

	/**
	 * Zooms out the camera to the game map state zoom.
	 */
	public void zoomOut() {
		if (!inPosition()) {
			moveTo();
		}
		spriteCam.zoom = Math.min(spriteCam.zoom + 0.01f, 1.5f);
//		spriteCam.position.x = Math.max(Math.min(spriteCam.position.x, 1920 - getWidth()/2.0f - 257), getWidth()/2.0f + 257);
//		spriteCam.position.y = Math.max(Math.min(spriteCam.position.y, 1080 - getHeight()/2.0f - 145), getHeight()/2.0f + 145);
		spriteCam.update();
	}

	/**
	 * Updates the cam by x and y amount.
	 */
	public void moveCam(float x, float y, float width) {
		if (x!=0 || y!=0) {
			float xIncr = spriteCam.position.x + x*10;
			float yIncr = spriteCam.position.y + y*10;
			spriteCam.position.x = Math.max(Math.min(xIncr, 1920 - getWidth()/2.0f - 257), getWidth()/2.0f + 257);
			spriteCam.position.y = Math.max(Math.min(yIncr, 1080 - getHeight()/2.0f - 15f), getHeight()/2.0f + 273);
			spriteCam.update();
		}
	}

	/**
	 * Resets the zoom to the game play state zoom.
	 */
	public void resetPrevZoom() {
		spriteCam.zoom = initialZoom;
		spriteCam.position.x = prevX;
		spriteCam.position.y = prevY;
		spriteCam.update();
	}
...
//		spriteCam.position.x = prevX;
//		spriteCam.position.y = prevY;
		spriteCam.zoom = Math.max(spriteCam.zoom - 0.01f, initialZoom);
		spriteCam.update();
	}

	public void moveBack() {
		spriteCam.position.x -= (spriteCam.position.x-prevX)/13.0f;
		spriteCam.position.y -= (spriteCam.position.y-prevY)/13.0f;
		spriteCam.update();
	}

	public void moveTo() {
		float posX = Math.max(Math.min(spriteCam.position.x, 1920 - getWidth()*initialZoom/2.0f - 257), getWidth()*initialZoom/2.0f + 257);
		float posY = Math.max(Math.min(spriteCam.position.y, 1080 - getHeight()*initialZoom/2.0f - 15f), getHeight()*initialZoom/2.0f + 273);
		spriteCam.position.x -= (spriteCam.position.x-posX)/15.0f;
		spriteCam.position.y -= (spriteCam.position.y-posY)/15.0f;
		spriteCam.update();
	}

	/**
	 * Returns true if the camera is at the default game play state zoom.
	 */
	public boolean inPosition() {
		float posX = Math.max(Math.min(spriteCam.position.x, 1920.0f - getWidth()/2.0f - 257.0f), getWidth()/2.0f + 257);
		float posY = Math.max(Math.min(spriteCam.position.y, 1080.0f - getHeight()/2.0f - 15f), getHeight()/2.0f + 273);
		if (Math.abs(spriteCam.position.x - posX)<=5.0f && Math.abs(spriteCam.position.y - posY)<=5.0f) {
			spriteCam.position.x = posX;
			spriteCam.position.y = posY;
			return true;
		}
		return false;
	}
	...
	/**
	 * Resets the zoom to the initial game zoom.
	 */
	public void resetZoom() {
		spriteCam.position.x = initialX;
		spriteCam.position.y = initialY;
		spriteCam.zoom = initialZoom;
		spriteCam.update();
	}

	/**
	 * Returns true if the zoom is at the default game play state zoom.
	 */
	public boolean doneZooming() {
		return spriteCam.zoom == initialZoom;
	}

	/**
	 * Returns true if the camera is at the default game play state zoom.
	 */
	public boolean doneMovingBack() {
		if (Math.abs(spriteCam.position.x - prevX)<=5.0f && Math.abs(spriteCam.position.y - prevY)<=5.0f) {
			spriteCam.position.x = prevX;
			spriteCam.position.y = prevY;
			return true;
		}
