...
public class LevelSelect implements Screen, InputProcessor, ControllerListener {
    ...
    public LevelSelect(GameCanvas canvas, AssetManager manager, SavedGameLoader savedGameLoader, LevelLoader levelLoader, int millis) {
        ...
        levels = savedGameLoader.getJustAvailableLevels();
        ...
        num = 0;
        for (Level level : savedGameLoader.getCompletedLevels()) {
            Texture button = new Texture(COMPLETED_FILE);
            button.setFilter(Texture.TextureFilter.Linear, Texture.TextureFilter.Linear);
            buttons.add(button);
            Texture clickedButton = new Texture(COMPLETED_CLICKED_FILE);
        ...
        for (Level level : savedGameLoader.getJustAvailableLevels()) {
            Texture button = new Texture(AVAILABLE_FILE);
            button.setFilter(Texture.TextureFilter.Linear, Texture.TextureFilter.Linear);
            buttons.add(button);
            Texture clickedButton = new Texture(AVAILABLE_CLICKED_FILE);
            clickedButton.setFilter(Texture.TextureFilter.Linear, Texture.TextureFilter.Linear);
            clickedButtons.add(clickedButton);
            Texture number = new Texture(levelLoader.getImagePath(String.valueOf(num + 1)));
            number.setFilter(Texture.TextureFilter.Linear, Texture.TextureFilter.Linear);
        ...
        for (int i = 0; i < num; i++) {
            if (i < positions.size && i < buttons.size) {
                ...
                if (buttons.size > i) {
                    ...
                    canvas.draw(b, tint, b.getWidth() / 2, b.getHeight() / 2,
                            x, y, 0, BUTTON_SCALE * scale, BUTTON_SCALE * scale);
                    Texture n = numbers.get(i);
                    canvas.draw(n, tint, n.getWidth() / 2, n.getHeight() / 2,
                            x, y, 0, BUTTON_SCALE * scale, BUTTON_SCALE * scale);
                }
            }
        }
...
//        canvas.draw(n, tint, n.getWidth() / 2, n.getHeight() / 2, x, y, 0, BUTTON_SCALE * scale, BUTTON_SCALE * scale);
    ...
    public void resize(int width, int height) {
        ...
        positions.add(new Vector2(width/2 - 4, (int)(BAR_HEIGHT_RATIO*height) - 22)); //2
        positions.add(new Vector2(width/2 + 112, (int)(BAR_HEIGHT_RATIO*height) - 45)); //3
        positions.add(new Vector2(width/2 + 274, (int)(BAR_HEIGHT_RATIO*height)+4)); //4
        ...
        positions.add(new Vector2(width/2 - 155, (int)(BAR_HEIGHT_RATIO*height)+68)); //7
        positions.add(new Vector2(width/2 - 282, (int)(BAR_HEIGHT_RATIO*height)+5)); //8
        positions.add(new Vector2(width/2 - 423, (int)(BAR_HEIGHT_RATIO*height)+75)); //9
    }
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        ...
        for (int i = 0; i < num; i++) {
            if (i < positions.size && i < buttons.size) {
                ...
                float radius = BUTTON_SCALE*scale*b.getWidth()/2.0f;
                float x = positions.get(i).x;
                float y = positions.get(i).y;
