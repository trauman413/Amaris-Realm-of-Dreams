...
public class FountainModel extends BoxObstacle {
    ...
    public FountainModel(float x, float y, float width, float height, FountainType fountain) {
        super(x,y,width*0.5f, height*0.7f);
 ...
 */
    public void draw(GameCanvas canvas) {
        ...
        canvas.draw(currentFrame, color, origin.x,origin.y,getX()*drawScale.x+50,getY()*drawScale.y+60, texture.getRegionWidth()/FRAME_COLS, texture.getRegionHeight()/FRAME_COLS);
        if (isAvailable && icon != null) {
            canvas.draw(icon, color, origin.x,origin.y,getX()*drawScale.x+75,getY()*drawScale.y+150, texture.getRegionWidth()/4.0f, texture.getRegionHeight()/4.0f);
        }
    }
}
